...
# Table of Contents
- [Zōrduzd: Nuclear Option Moza FFB Support](#zōrduzd-nuclear-option-moza-ffb-support)
- [Table of Contents](#table-of-contents)
- [How to Use](#how-to-use)
  - [Using Nuclear Option Mod Manager (NOMM)](#using-nuclear-option-mod-manager-nomm)
  - [Configuring Moza Cockpit](#configuring-moza-cockpit)
  - [Configuring the Ports](#configuring-the-ports)
  - [Configuring the FFB Feel](#configuring-the-ffb-feel)
  - [Manual Download](#manual-download)
  - [How it works](#how-it-works)
    - [A BepInEx plugin](#a-bepinex-plugin)
    - [A DCS "Faker"](#a-dcs-faker)
  - [Building](#building)
    - [Requirements](#requirements)
    - [Build](#build)
- [Inspiration \& Credit](#inspiration--credit)
  - [Falcon BMS to Moza FFB Mapper](#falcon-bms-to-moza-ffb-mapper)
  - [NOFFB (Force Feedback Plugin for Nuclear Option)](#noffb-force-feedback-plugin-for-nuclear-option)

# How to Use
## Using Nuclear Option Mod Manager (NOMM)
If you have [NOMM](https://github.com/Combat787/NOMM/releases), then the setup process is quite easy, simply look for `zorduzd` in NOMM and install it.
...
```
(of course, you might have a different path based on the game install location.)
...
__⚠️ IMPORTANT__:
__You need to run `DCS.exe` at the same time the game is running to enable FFB.__
Basically, this program tries to make Moza Cockpit _think_ that [DCS](https://www.digitalcombatsimulator.com/en/) is running.
This way, by sending good telemetry data to Moza Cockpit, we can make it apply force feedback to your flight stick.

## Configuring Moza Cockpit

If you have DCS installed on your system, and you've configured Moza Cockpit with it, then you are good to go.
Otherwise, run `DCS.exe` from the mod (under `Zorduzd\DCS World\bin\DCS.exe`).
...
| moza cockpit   | whether Moza Cockpit successfully connected to the app                    | Yes, see [configuring moza cockpit](#configuring-moza-cockpit) |
| nuclear option | whether the game is running and connected to the app                      | No                                                             |
| DCS            | whether you have a valid DCS setup with Moza Cockpit                      | Yes, see [configuring moza cockpit](#configuring-moza-cockpit) |

If you're here, then `DCS` is probably indicated as __offline__, and a `setup DCS` button must be clickable.
Click on it!
This will create a `DCS` folder under `%USERPROFILE%\Saved Games`, e.g., that's `C:\Users\John Doe\Saved Games`.
...
The _Game Installation Path_ must point to the `DCS World` directory installed by the mod, e.g., for me, that's
```
...
Now if you run the game, _nuclear option_ must also become online and you should get some force feedback.

To better adjust the force feedback, you have to play with [Moza cockpit profiles](#configuring-the-ffb-feel).



## Configuring the Ports
You should probably never need to do this, but you can change the ports over which the game and the fake `DCS` app communicate.
...
2. `DCS.exe`: before starting the program, modify `zorduzd.cfg` (if it does not exist, run `DCS.exe` once and quit) to set your desired ports. `moza` must match the port that Moza Cockpit sets for DCS and the `game` port must match the one from step 1.



## Configuring the FFB Feel
To configure how the FFB feels, you must open Moza Cockpit and load or create your profile.
At the moment, the mod makes Moza Cockpit believe that DCS is running and the user is flying an A-10C-II Warthog.
This is probably a good representation for the A-19 aircraft in Nuclear Option, but for others you must create and load your desired profile.
Additionally, Moza does not enable lots of cool FFB effects like wind turbulence, or even weapon discharge, so even with the default A-10C-II config, you might want to turn on and adjust more effects.
...
1. Listens on the Moza Cockpit's expected DCS port (`3476`) and waits for Moza Cockpit to connect.
...
The name `DCS.exe` is intentional: Moza Cockpit auto-detects running processes by name to enable its FFB profiles.
...
I was inspired by this trick to make Zorduzd without having to deal with calculating effects myself.
